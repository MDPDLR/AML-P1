...
Predicting whether an applicant will obtain a loan is therefore a useful and socially relevant problem, which also provides an interesting case study for applying probabilistic and linear models in machine learning.\\

%    (3) Your approach: what methods from the course are you applying? (mention Part I/II/III)
The dataset used in this work includes both numerical and categorical features describing applicants’ profiles and their corresponding loan approval outcomes. 
Such data naturally involves uncertainty and feature correlations, making it suitable for exploring the concepts introduced in \emph{Part I} of the course, namely Bayesian reasoning, Generalized Linear Models (GLMs), and linear discriminant methods.\\
...
Section~\ref{sec:related} briefly reviews related works and previous approaches. 
...
The dataset, entitled \emph{\href{https://www.kaggle.com/datasets/anishdevedward/loan-approval-dataset}{Loan Approval Dataset}}, was obtained from an open-access financial repository on Kaggle and contains information about applicants and their corresponding loan approval status. 
It includes both numerical and categorical features that describe socio-economic and financial characteristics such as income, credit score, employment type, and marital status. 
...
After removing these variables, the dataset contained a mixture of continuous and categorical attributes suitable for the models considered in this project.\\

No missing data or outliers were detected, which simplified preprocessing and ensured that no imputation or cleaning procedures were necessary. 
The class distribution was relatively balanced, meaning that no reweighting or resampling techniques were required.
...
As shown in Figure~\ref{fig:boxplots}, 
loan approval is strongly associated with higher values of \texttt{income}, \texttt{credit\_score}, and \texttt{points}, 
confirming the relevance of financial stability indicators in credit assessment. 
...
To assess potential redundancy and feature interdependence, 
the Pearson correlation matrix was computed for all numerical variables (Figure~\ref{fig:corr}). 
The analysis reveals a strong positive correlation ($r \approx 0.74$) between \texttt{credit\_score} and \texttt{points}, 
and a moderate one ($r \approx 0.45$) between \texttt{income} and \texttt{points}. 
These relationships are expected given the shared financial nature of the indicators. 
Importantly, the correlation values remain below critical thresholds, 
indicating no severe multicollinearity that would compromise model interpretability or stability.\\
...
The insights gained here, particularly the correlation between key financial features, will be directly leveraged in the modeling phase to interpret each algorithm’s behavior.
